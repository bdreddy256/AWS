...
def export_firewall(host, apikey, out_dir, verify_tls=False, label=None):
    ...
    print(f"      Pulling pushed-shared-policy (Panorama rules + profile-groups)...")
    panorama_elem = None
    try:
        r = _api_get(host, {
            "type": "op",
        ...
        # API returns <response><result><policy><panorama>...  (element names can vary
        # slightly by PAN-OS version). We locate the <panorama> element wherever it is
        # and normalize to  config/pushed_policy/panorama  which the checks expect.
        panorama_elem = r.find(".//panorama")
        if panorama_elem is None:
            # some versions wrap under <policy> directly with rulebases as children
            policy = r.find(".//result/policy") or r.find(".//result")
            panorama_elem = policy  # treat its children as the panorama contents
        if panorama_elem is not None:
            n_rules = len(panorama_elem.findall(".//security/rules/entry"))
            n_pg = len(panorama_elem.findall(".//profile-group/entry"))
            print(f"      ✓ pushed-shared-policy retrieved "
                  f"({n_rules} security rules, {n_pg} profile-groups)")
        else:
            print(f"      ! pushed-shared-policy: could not locate policy content")
    except Exception as e:
        print(f"      ! pushed-shared-policy error: {e}")

    # --- 3. Merge: normalize to config/pushed_policy/panorama ---
    if panorama_elem is not None:
        pushed_elem = ET.SubElement(merged_cfg, "pushed_policy")
        if panorama_elem.tag == "panorama":
            pushed_elem.append(panorama_elem)
        else:
            # wrap children under a synthetic <panorama> so paths line up
            wrapper = ET.SubElement(pushed_elem, "panorama")
            for child in panorama_elem:
