...
def chk_6_1_av_reset_both(root):
    ...
    for name, prof in in_use:
        ...
        weak = [dn for dn, a in actions.items()
                if dn not in EXEMPT and a != "reset-both"]
        shown = ", ".join(f"{dn}={a}" for dn, a in list(actions.items())[:4])
        if actions:
            details.append(f"{name}[{shown}{'...' if len(actions)>4 else ''}]")
            if weak:
                all_ok = False
        else:
    ...
    current = "; ".join(details[:2])
    req = "CIS: reset-both on all decoders except imap/pop3"
    if all_ok:
        return "PASS", f"Current: {current}. {req} → met"
    return "FAIL", f"Current: {current}. {req} → some decoders not reset-both"
...
def chk_6_3_anti_spyware_block(root):
    ...
    BLOCK = {"reset-both", "reset-client", "reset-server", "drop", "block-ip", "block"}
    details, all_ok = [], True
    for name, prof in in_use:
        rules = prof.findall(".//rules/entry")
        sev_action = []
        for r in rules:
            # action is the first child under <action> (e.g. <alert/>, <reset-both/>, <drop/>)
        ...
        weak = [f"{rn}({sv})={ac}" for rn, sv, ac in sev_action if ac not in BLOCK]
        shown = "; ".join(f"{rn}={ac}" for rn, sv, ac in sev_action[:3])
        details.append(f"{name}[{shown}]")
        if weak or not sev_action:
            all_ok = False

    current = " | ".join(details[:2])
    req = "CIS: block/reset on all severities"
    if all_ok:
        return "PASS", f"Current: {current}. {req} → met"
...
def chk_6_6_vulnerability_block(root):
    ...
    for name, prof in in_use:
        ...
        # critical & high must include a blocking action
        problems = []
        for sev in HIGHSEV:
            acts = by_sev.get(sev, set())
            if not acts or not (acts & BLOCK):
...
def chk_6_20_wildfire_ml_av(root):
    ...
    for name, prof in in_use:
        ...
        for d in prof.findall(".//decoder/entry"):
            ...
            if wf:
                vals.append((dn, wf))
        weak = [f"{dn}={wf}" for dn, wf in vals if wf != "reset-both"]
        shown = ", ".join(f"{dn}={wf}" for dn, wf in vals[:3])
        details.append(f"{name}[{shown or 'no wildfire-action set'}]")
        if weak or not vals:
            all_ok = False
    current = " | ".join(details[:2])
    req = "CIS: wildfire-action=reset-both (except imap/pop3)"
    return ("PASS" if all_ok else "FAIL"), f"Current: {current}. {req}"
