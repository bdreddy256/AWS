...
READ-ONLY: only issues 'show config' / 'show' op API calls. Never commits.
...
# ------------------- WildFire op-state (CIS 5.1) -------------------
#
# CIS 5.1 requires WildFire file-size upload limits set to MAX. These live in
# operational state, not config (defaults aren't written to config). So we pull
# 'show wildfire status', parse the 'File size limit info:' block, and inject it
# under cfg as  op_state/wildfire/file-size-limits  so chk_5_1 can read effective
# values (default OR overridden) for every device.

def _fetch_wildfire_status(host, apikey, verify_tls=False, target=None):
    """Pull 'show wildfire status' (op) and return raw flattened text, or None."""
    params = {"type": "op",
              "cmd": "<show><wildfire><status></status></wildfire></show>",
              "key": apikey}
    if target:
        params["target"] = target
    try:
        ...
        print(f"    ! wildfire status pull failed: {e}", file=sys.stderr)
        return None
    # PAN-OS flattens the whole status dump into result/member (or result text).
    return "".join(root.itertext())


def _parse_wf_file_sizes(text):
    """
    Extract 'File size limit info:' entries from show-wildfire-status text.
    Returns list of (type, value, unit). Handles the single-line flattened form.
    """
    if not text:
        return []
    m = re.search(r"File size limit info:(.*?)(?:Forwarding info:|Public Cloud:|$)",
                  text, re.IGNORECASE | re.DOTALL)
    seg = m.group(1) if m else text
    pairs = re.findall(r"([A-Za-z\-]+)\s+(\d+)\s*(MB|KB|GB)", seg)
    known = {"pe", "apk", "pdf", "ms-office", "jar", "flash", "macosx",
             "archive", "linux", "script", "eml", "email-link"}
    out = []
...
def _stitch_wildfire_status(cfg, text):
    """Inject parsed file-size limits as op_state/wildfire/file-size-limits."""
    sizes = _parse_wf_file_sizes(text)
    if cfg is None or not sizes:
        if cfg is not None and text and not sizes:
            print("    ! wildfire status: no file-size block parsed", file=sys.stderr)
        return cfg
    op = cfg.find("op_state")
    if op is None:
        op = ET.SubElement(cfg, "op_state")
    wf = ET.SubElement(op, "wildfire")
    fsl = ET.SubElement(wf, "file-size-limits")
    for name, val, unit in sizes:
        e = ET.SubElement(fsl, "entry")
        e.set("name", name)
        ET.SubElement(e, "value").text = val
        ET.SubElement(e, "unit").text = unit
    print(f"    wildfire file-size limits: {len(sizes)} types captured")
    return cfg
...
def get_firewall_running_config(host, apikey, verify_tls=False):
    ...
    if _config_has_policy(cfg):
        ...
        _report_pull(cfg)
        _stitch_wildfire_status(cfg, _fetch_wildfire_status(host, apikey, verify_tls))
        return cfg
...
def get_device_config_via_panorama(host, apikey, serial, verify_tls=False):
    ...
    if _config_has_policy(cfg):
        ...
        _report_pull(cfg)
        _stitch_wildfire_status(cfg, _fetch_wildfire_status(host, apikey, verify_tls, target=serial))
        return cfg
...
def main():
    ...
    # Version banner — confirms you are running the FIXED file.
    print("[*] cis_panos.audit  BUILD 2026-07-21-wf51  (WildFire 5.1 op-state fetch)")
