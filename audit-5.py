...
def get_firewall_running_config(host, apikey, verify_tls=False):
    # 1) MERGED config -> device settings (mgmt, password, NTP, SNMP, zones...)
    #    Merged includes Panorama template-pushed values (avoids false FAILs).
    root = _api_get(host, {
        "type": "op",
    ...
    if cfg is None:
        raise RuntimeError(f"No merged config returned from {host}")

    # 2) PUSHED-SHARED-POLICY -> security rules + profile-groups + profiles.
    #    Merged config does NOT contain these on a Panorama-managed FW, so we
    #    fetch them separately and stitch them in as pushed_policy/panorama.
    pano = _fetch_pushed_policy(host, apikey, verify_tls)
    n_rules = len(pano.findall(".//security/rules/entry")) if pano is not None else 0
    n_pg = len(pano.findall(".//profile-group/entry")) if pano is not None else 0
    print(f"    pushed-shared-policy: {n_rules} security rules, {n_pg} profile-groups")
...
def get_device_config_via_panorama(host, apikey, serial, verify_tls=False):
    # Merged (effective) config proxied via Panorama with target=<serial>.
    root = _api_get(host, {
        "type": "op",
        "cmd": "<show><config><merged></merged></config></show>",
    ...
    if cfg is None:
        cfg = root.find(".//result")
    if cfg is None:
        raise RuntimeError(f"No merged config for device {serial} via Panorama")

    # Pull pushed-shared-policy for the same device (proxied) and stitch in.
