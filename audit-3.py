...
def get_firewall_running_config(host, apikey, verify_tls=False):
    # Use MERGED config so Panorama template-pushed values are included.
    # Plain 'xpath=/config show' returns only locally-stored config and will
    # miss settings pushed from Panorama templates (causes false FAILs).
    root = _api_get(host, {
        "type": "op",
...
def get_device_config_via_panorama(host, apikey, serial, verify_tls=False):
    # Pull MERGED (effective) config from the device, proxied via Panorama with
    # target=<serial>. This includes template-pushed values, matching what runs
    # on the box. Plain '/config show' returns stored-only config (false FAILs).
    root = _api_get(host, {
        "type": "op",
        "cmd": "<show><config><merged></merged></config></show>",
